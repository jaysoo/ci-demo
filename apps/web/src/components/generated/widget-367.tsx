...
export interface Sample367 {
  tick: number;
  value: number;
}

export function pipeline367(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 367 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample367, Sample367[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample367[])),
  );
}

export function windows367() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream367({ seed = 367 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample367[]>([]);

  useEffect(() => {
    const sub = pipeline367(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 367</p>
      {summary ? (
