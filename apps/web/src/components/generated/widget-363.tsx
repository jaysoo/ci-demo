...
export interface Sample363 {
  tick: number;
  value: number;
}

export function pipeline363(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 363 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample363, Sample363[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample363[])),
  );
}

export function windows363() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream363({ seed = 363 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample363[]>([]);

  useEffect(() => {
    const sub = pipeline363(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 363</p>
      {summary ? (
