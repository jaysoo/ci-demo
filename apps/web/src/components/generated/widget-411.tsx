...
export interface Sample411 {
  tick: number;
  value: number;
}

export function pipeline411(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 411 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample411, Sample411[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample411[])),
  );
}

export function windows411() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream411({ seed = 411 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample411[]>([]);

  useEffect(() => {
    const sub = pipeline411(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 411</p>
      {summary ? (
