...
export interface Sample595 {
  tick: number;
  value: number;
}

export function pipeline595(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 595 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample595, Sample595[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample595[])),
  );
}

export function windows595() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream595({ seed = 595 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample595[]>([]);

  useEffect(() => {
    const sub = pipeline595(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 595</p>
      {summary ? (
