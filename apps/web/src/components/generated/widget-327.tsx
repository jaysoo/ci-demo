...
export interface Sample327 {
  tick: number;
  value: number;
}

export function pipeline327(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 327 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample327, Sample327[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample327[])),
  );
}

export function windows327() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream327({ seed = 327 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample327[]>([]);

  useEffect(() => {
    const sub = pipeline327(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 327</p>
      {summary ? (
