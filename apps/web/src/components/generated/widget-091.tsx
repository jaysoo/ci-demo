...
export interface Sample091 {
  tick: number;
  value: number;
}

export function pipeline091(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 91 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample091, Sample091[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample091[])),
  );
}

export function windows091() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream091({ seed = 91 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample091[]>([]);

  useEffect(() => {
    const sub = pipeline091(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 091</p>
      {summary ? (
