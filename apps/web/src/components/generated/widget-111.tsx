...
export interface Sample111 {
  tick: number;
  value: number;
}

export function pipeline111(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 111 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample111, Sample111[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample111[])),
  );
}

export function windows111() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream111({ seed = 111 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample111[]>([]);

  useEffect(() => {
    const sub = pipeline111(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 111</p>
      {summary ? (
