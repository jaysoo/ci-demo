...
export interface Panel600Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel600Data(size = 48): Panel600Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (600 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel600Datum) => d.value);

export function Panel600({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel600Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel600Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 600</h3>
