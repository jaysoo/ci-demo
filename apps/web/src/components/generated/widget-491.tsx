...
export interface Sample491 {
  tick: number;
  value: number;
}

export function pipeline491(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 491 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample491, Sample491[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample491[])),
  );
}

export function windows491() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream491({ seed = 491 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample491[]>([]);

  useEffect(() => {
    const sub = pipeline491(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 491</p>
      {summary ? (
