...
export interface Sample195 {
  tick: number;
  value: number;
}

export function pipeline195(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 195 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample195, Sample195[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample195[])),
  );
}

export function windows195() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream195({ seed = 195 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample195[]>([]);

  useEffect(() => {
    const sub = pipeline195(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 195</p>
      {summary ? (
