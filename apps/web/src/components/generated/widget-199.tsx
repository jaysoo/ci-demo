...
export interface Sample199 {
  tick: number;
  value: number;
}

export function pipeline199(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 199 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample199, Sample199[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample199[])),
  );
}

export function windows199() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream199({ seed = 199 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample199[]>([]);

  useEffect(() => {
    const sub = pipeline199(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 199</p>
      {summary ? (
