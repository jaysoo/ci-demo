...
export interface Sample087 {
  tick: number;
  value: number;
}

export function pipeline087(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 87 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample087, Sample087[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample087[])),
  );
}

export function windows087() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream087({ seed = 87 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample087[]>([]);

  useEffect(() => {
    const sub = pipeline087(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 087</p>
      {summary ? (
