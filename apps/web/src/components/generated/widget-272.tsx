...
export interface Panel272Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel272Data(size = 48): Panel272Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (272 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel272Datum) => d.value);

export function Panel272({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel272Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel272Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 272</h3>
