...
export interface Sample123 {
  tick: number;
  value: number;
}

export function pipeline123(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 123 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample123, Sample123[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample123[])),
  );
}

export function windows123() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream123({ seed = 123 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample123[]>([]);

  useEffect(() => {
    const sub = pipeline123(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 123</p>
      {summary ? (
