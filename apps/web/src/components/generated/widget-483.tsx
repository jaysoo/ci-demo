...
export interface Sample483 {
  tick: number;
  value: number;
}

export function pipeline483(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 483 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample483, Sample483[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample483[])),
  );
}

export function windows483() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream483({ seed = 483 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample483[]>([]);

  useEffect(() => {
    const sub = pipeline483(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 483</p>
      {summary ? (
