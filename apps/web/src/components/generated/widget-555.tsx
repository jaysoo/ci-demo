...
export interface Sample555 {
  tick: number;
  value: number;
}

export function pipeline555(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 555 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample555, Sample555[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample555[])),
  );
}

export function windows555() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream555({ seed = 555 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample555[]>([]);

  useEffect(() => {
    const sub = pipeline555(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 555</p>
      {summary ? (
