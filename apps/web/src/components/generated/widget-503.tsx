...
export interface Sample503 {
  tick: number;
  value: number;
}

export function pipeline503(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 503 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample503, Sample503[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample503[])),
  );
}

export function windows503() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream503({ seed = 503 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample503[]>([]);

  useEffect(() => {
    const sub = pipeline503(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 503</p>
      {summary ? (
