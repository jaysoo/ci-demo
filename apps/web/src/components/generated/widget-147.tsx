...
export interface Sample147 {
  tick: number;
  value: number;
}

export function pipeline147(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 147 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample147, Sample147[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample147[])),
  );
}

export function windows147() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream147({ seed = 147 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample147[]>([]);

  useEffect(() => {
    const sub = pipeline147(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 147</p>
      {summary ? (
