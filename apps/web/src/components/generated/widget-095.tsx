...
export interface Sample095 {
  tick: number;
  value: number;
}

export function pipeline095(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 95 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample095, Sample095[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample095[])),
  );
}

export function windows095() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream095({ seed = 95 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample095[]>([]);

  useEffect(() => {
    const sub = pipeline095(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 095</p>
      {summary ? (
