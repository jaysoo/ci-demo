...
export interface Sample527 {
  tick: number;
  value: number;
}

export function pipeline527(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 527 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample527, Sample527[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample527[])),
  );
}

export function windows527() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream527({ seed = 527 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample527[]>([]);

  useEffect(() => {
    const sub = pipeline527(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 527</p>
      {summary ? (
