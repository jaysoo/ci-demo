...
export interface Sample255 {
  tick: number;
  value: number;
}

export function pipeline255(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 255 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample255, Sample255[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample255[])),
  );
}

export function windows255() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream255({ seed = 255 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample255[]>([]);

  useEffect(() => {
    const sub = pipeline255(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 255</p>
      {summary ? (
