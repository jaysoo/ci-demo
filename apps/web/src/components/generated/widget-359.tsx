...
export interface Sample359 {
  tick: number;
  value: number;
}

export function pipeline359(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 359 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample359, Sample359[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample359[])),
  );
}

export function windows359() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream359({ seed = 359 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample359[]>([]);

  useEffect(() => {
    const sub = pipeline359(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 359</p>
      {summary ? (
