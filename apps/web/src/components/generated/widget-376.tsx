...
export interface Panel376Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel376Data(size = 48): Panel376Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (376 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel376Datum) => d.value);

export function Panel376({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel376Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel376Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 376</h3>
