...
export interface Sample599 {
  tick: number;
  value: number;
}

export function pipeline599(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 599 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample599, Sample599[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample599[])),
  );
}

export function windows599() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream599({ seed = 599 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample599[]>([]);

  useEffect(() => {
    const sub = pipeline599(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 599</p>
      {summary ? (
