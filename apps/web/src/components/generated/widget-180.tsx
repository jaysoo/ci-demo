...
export interface Panel180Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel180Data(size = 48): Panel180Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (180 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel180Datum) => d.value);

export function Panel180({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel180Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel180Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 180</h3>
