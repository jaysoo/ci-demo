...
export interface Sample515 {
  tick: number;
  value: number;
}

export function pipeline515(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 515 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample515, Sample515[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample515[])),
  );
}

export function windows515() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream515({ seed = 515 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample515[]>([]);

  useEffect(() => {
    const sub = pipeline515(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 515</p>
      {summary ? (
