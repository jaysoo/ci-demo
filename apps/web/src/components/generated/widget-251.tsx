...
export interface Sample251 {
  tick: number;
  value: number;
}

export function pipeline251(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 251 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample251, Sample251[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample251[])),
  );
}

export function windows251() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream251({ seed = 251 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample251[]>([]);

  useEffect(() => {
    const sub = pipeline251(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 251</p>
      {summary ? (
