...
export interface Panel228Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel228Data(size = 48): Panel228Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (228 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel228Datum) => d.value);

export function Panel228({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel228Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel228Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 228</h3>
