...
export interface Sample043 {
  tick: number;
  value: number;
}

export function pipeline043(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 43 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample043, Sample043[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample043[])),
  );
}

export function windows043() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream043({ seed = 43 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample043[]>([]);

  useEffect(() => {
    const sub = pipeline043(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 043</p>
      {summary ? (
