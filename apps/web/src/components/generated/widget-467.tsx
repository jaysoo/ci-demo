...
export interface Sample467 {
  tick: number;
  value: number;
}

export function pipeline467(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 467 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample467, Sample467[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample467[])),
  );
}

export function windows467() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream467({ seed = 467 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample467[]>([]);

  useEffect(() => {
    const sub = pipeline467(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 467</p>
      {summary ? (
