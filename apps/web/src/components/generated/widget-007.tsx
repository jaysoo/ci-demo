...
export interface Sample007 {
  tick: number;
  value: number;
}

export function pipeline007(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 7 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample007, Sample007[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample007[])),
  );
}

export function windows007() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream007({ seed = 7 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample007[]>([]);

  useEffect(() => {
    const sub = pipeline007(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 007</p>
      {summary ? (
