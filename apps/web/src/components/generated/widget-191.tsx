...
export interface Sample191 {
  tick: number;
  value: number;
}

export function pipeline191(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 191 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample191, Sample191[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample191[])),
  );
}

export function windows191() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream191({ seed = 191 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample191[]>([]);

  useEffect(() => {
    const sub = pipeline191(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 191</p>
      {summary ? (
