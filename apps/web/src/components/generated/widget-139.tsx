...
export interface Sample139 {
  tick: number;
  value: number;
}

export function pipeline139(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 139 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample139, Sample139[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample139[])),
  );
}

export function windows139() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream139({ seed = 139 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample139[]>([]);

  useEffect(() => {
    const sub = pipeline139(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 139</p>
      {summary ? (
