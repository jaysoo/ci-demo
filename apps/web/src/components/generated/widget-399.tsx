...
export interface Sample399 {
  tick: number;
  value: number;
}

export function pipeline399(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 399 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample399, Sample399[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample399[])),
  );
}

export function windows399() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream399({ seed = 399 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample399[]>([]);

  useEffect(() => {
    const sub = pipeline399(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 399</p>
      {summary ? (
