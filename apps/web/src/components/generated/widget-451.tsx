...
export interface Sample451 {
  tick: number;
  value: number;
}

export function pipeline451(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 451 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample451, Sample451[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample451[])),
  );
}

export function windows451() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream451({ seed = 451 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample451[]>([]);

  useEffect(() => {
    const sub = pipeline451(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 451</p>
      {summary ? (
