...
export interface Sample543 {
  tick: number;
  value: number;
}

export function pipeline543(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 543 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample543, Sample543[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample543[])),
  );
}

export function windows543() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream543({ seed = 543 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample543[]>([]);

  useEffect(() => {
    const sub = pipeline543(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 543</p>
      {summary ? (
