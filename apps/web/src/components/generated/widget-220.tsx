...
export interface Panel220Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel220Data(size = 48): Panel220Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (220 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel220Datum) => d.value);

export function Panel220({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel220Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel220Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 220</h3>
