...
export interface Sample471 {
  tick: number;
  value: number;
}

export function pipeline471(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 471 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample471, Sample471[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample471[])),
  );
}

export function windows471() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream471({ seed = 471 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample471[]>([]);

  useEffect(() => {
    const sub = pipeline471(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 471</p>
      {summary ? (
