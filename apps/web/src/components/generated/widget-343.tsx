...
export interface Sample343 {
  tick: number;
  value: number;
}

export function pipeline343(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 343 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample343, Sample343[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample343[])),
  );
}

export function windows343() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream343({ seed = 343 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample343[]>([]);

  useEffect(() => {
    const sub = pipeline343(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 343</p>
      {summary ? (
