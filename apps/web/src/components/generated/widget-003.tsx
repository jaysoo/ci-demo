...
export interface Sample003 {
  tick: number;
  value: number;
}

export function pipeline003(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 3 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample003, Sample003[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample003[])),
  );
}

export function windows003() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream003({ seed = 3 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample003[]>([]);

  useEffect(() => {
    const sub = pipeline003(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 003</p>
      {summary ? (
