...
export interface Panel420Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel420Data(size = 48): Panel420Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (420 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel420Datum) => d.value);

export function Panel420({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel420Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel420Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 420</h3>
