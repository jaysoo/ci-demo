...
export interface Sample027 {
  tick: number;
  value: number;
}

export function pipeline027(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 27 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample027, Sample027[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample027[])),
  );
}

export function windows027() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream027({ seed = 27 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample027[]>([]);

  useEffect(() => {
    const sub = pipeline027(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 027</p>
      {summary ? (
