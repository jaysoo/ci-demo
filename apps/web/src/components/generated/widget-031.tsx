...
export interface Sample031 {
  tick: number;
  value: number;
}

export function pipeline031(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 31 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample031, Sample031[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample031[])),
  );
}

export function windows031() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream031({ seed = 31 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample031[]>([]);

  useEffect(() => {
    const sub = pipeline031(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 031</p>
      {summary ? (
