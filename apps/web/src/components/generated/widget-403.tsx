...
export interface Sample403 {
  tick: number;
  value: number;
}

export function pipeline403(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 403 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample403, Sample403[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample403[])),
  );
}

export function windows403() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream403({ seed = 403 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample403[]>([]);

  useEffect(() => {
    const sub = pipeline403(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 403</p>
      {summary ? (
