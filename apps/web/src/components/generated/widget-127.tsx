...
export interface Sample127 {
  tick: number;
  value: number;
}

export function pipeline127(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 127 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample127, Sample127[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample127[])),
  );
}

export function windows127() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream127({ seed = 127 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample127[]>([]);

  useEffect(() => {
    const sub = pipeline127(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 127</p>
      {summary ? (
