...
export interface Panel332Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel332Data(size = 48): Panel332Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (332 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel332Datum) => d.value);

export function Panel332({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel332Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel332Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 332</h3>
