...
export interface Sample187 {
  tick: number;
  value: number;
}

export function pipeline187(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 187 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample187, Sample187[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample187[])),
  );
}

export function windows187() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream187({ seed = 187 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample187[]>([]);

  useEffect(() => {
    const sub = pipeline187(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 187</p>
      {summary ? (
