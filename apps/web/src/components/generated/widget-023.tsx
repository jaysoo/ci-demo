...
export interface Sample023 {
  tick: number;
  value: number;
}

export function pipeline023(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 23 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample023, Sample023[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample023[])),
  );
}

export function windows023() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream023({ seed = 23 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample023[]>([]);

  useEffect(() => {
    const sub = pipeline023(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 023</p>
      {summary ? (
