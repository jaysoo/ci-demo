...
export interface Sample523 {
  tick: number;
  value: number;
}

export function pipeline523(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 523 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample523, Sample523[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample523[])),
  );
}

export function windows523() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream523({ seed = 523 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample523[]>([]);

  useEffect(() => {
    const sub = pipeline523(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 523</p>
      {summary ? (
