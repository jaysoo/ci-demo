...
export interface Sample047 {
  tick: number;
  value: number;
}

export function pipeline047(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 47 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample047, Sample047[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample047[])),
  );
}

export function windows047() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream047({ seed = 47 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample047[]>([]);

  useEffect(() => {
    const sub = pipeline047(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 047</p>
      {summary ? (
