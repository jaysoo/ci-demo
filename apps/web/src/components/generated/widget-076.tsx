...
export interface Panel076Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel076Data(size = 48): Panel076Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (76 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel076Datum) => d.value);

export function Panel076({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel076Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel076Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 076</h3>
