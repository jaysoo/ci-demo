...
export interface Panel564Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel564Data(size = 48): Panel564Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (564 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel564Datum) => d.value);

export function Panel564({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel564Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel564Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 564</h3>
