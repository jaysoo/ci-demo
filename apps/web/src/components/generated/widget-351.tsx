...
export interface Sample351 {
  tick: number;
  value: number;
}

export function pipeline351(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 351 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample351, Sample351[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample351[])),
  );
}

export function windows351() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream351({ seed = 351 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample351[]>([]);

  useEffect(() => {
    const sub = pipeline351(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 351</p>
      {summary ? (
