...
export interface Sample335 {
  tick: number;
  value: number;
}

export function pipeline335(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 335 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample335, Sample335[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample335[])),
  );
}

export function windows335() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream335({ seed = 335 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample335[]>([]);

  useEffect(() => {
    const sub = pipeline335(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 335</p>
      {summary ? (
