...
export interface Sample039 {
  tick: number;
  value: number;
}

export function pipeline039(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 39 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample039, Sample039[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample039[])),
  );
}

export function windows039() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream039({ seed = 39 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample039[]>([]);

  useEffect(() => {
    const sub = pipeline039(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 039</p>
      {summary ? (
