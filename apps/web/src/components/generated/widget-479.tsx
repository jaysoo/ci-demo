...
export interface Sample479 {
  tick: number;
  value: number;
}

export function pipeline479(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 479 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample479, Sample479[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample479[])),
  );
}

export function windows479() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream479({ seed = 479 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample479[]>([]);

  useEffect(() => {
    const sub = pipeline479(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 479</p>
      {summary ? (
