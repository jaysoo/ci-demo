...
export interface Sample583 {
  tick: number;
  value: number;
}

export function pipeline583(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 583 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample583, Sample583[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample583[])),
  );
}

export function windows583() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream583({ seed = 583 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample583[]>([]);

  useEffect(() => {
    const sub = pipeline583(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 583</p>
      {summary ? (
