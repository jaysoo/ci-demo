...
export interface Sample319 {
  tick: number;
  value: number;
}

export function pipeline319(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 319 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample319, Sample319[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample319[])),
  );
}

export function windows319() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream319({ seed = 319 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample319[]>([]);

  useEffect(() => {
    const sub = pipeline319(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 319</p>
      {summary ? (
