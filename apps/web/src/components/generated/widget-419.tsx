...
export interface Sample419 {
  tick: number;
  value: number;
}

export function pipeline419(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 419 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample419, Sample419[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample419[])),
  );
}

export function windows419() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream419({ seed = 419 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample419[]>([]);

  useEffect(() => {
    const sub = pipeline419(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 419</p>
      {summary ? (
