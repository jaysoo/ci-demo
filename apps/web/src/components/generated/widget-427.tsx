...
export interface Sample427 {
  tick: number;
  value: number;
}

export function pipeline427(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 427 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample427, Sample427[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample427[])),
  );
}

export function windows427() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream427({ seed = 427 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample427[]>([]);

  useEffect(() => {
    const sub = pipeline427(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 427</p>
      {summary ? (
