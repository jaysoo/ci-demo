...
export interface Panel460Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel460Data(size = 48): Panel460Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (460 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel460Datum) => d.value);

export function Panel460({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel460Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel460Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 460</h3>
