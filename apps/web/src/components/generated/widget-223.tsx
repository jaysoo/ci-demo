...
export interface Sample223 {
  tick: number;
  value: number;
}

export function pipeline223(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 223 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample223, Sample223[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample223[])),
  );
}

export function windows223() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream223({ seed = 223 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample223[]>([]);

  useEffect(() => {
    const sub = pipeline223(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 223</p>
      {summary ? (
