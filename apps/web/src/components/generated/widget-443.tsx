...
export interface Sample443 {
  tick: number;
  value: number;
}

export function pipeline443(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 443 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample443, Sample443[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample443[])),
  );
}

export function windows443() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream443({ seed = 443 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample443[]>([]);

  useEffect(() => {
    const sub = pipeline443(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 443</p>
      {summary ? (
