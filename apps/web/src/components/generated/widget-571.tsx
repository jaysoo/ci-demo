...
export interface Sample571 {
  tick: number;
  value: number;
}

export function pipeline571(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 571 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample571, Sample571[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample571[])),
  );
}

export function windows571() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream571({ seed = 571 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample571[]>([]);

  useEffect(() => {
    const sub = pipeline571(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 571</p>
      {summary ? (
