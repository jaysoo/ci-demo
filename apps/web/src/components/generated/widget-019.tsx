...
export interface Sample019 {
  tick: number;
  value: number;
}

export function pipeline019(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 19 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample019, Sample019[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample019[])),
  );
}

export function windows019() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream019({ seed = 19 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample019[]>([]);

  useEffect(() => {
    const sub = pipeline019(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 019</p>
      {summary ? (
