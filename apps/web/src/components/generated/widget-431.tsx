...
export interface Sample431 {
  tick: number;
  value: number;
}

export function pipeline431(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 431 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample431, Sample431[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample431[])),
  );
}

export function windows431() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream431({ seed = 431 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample431[]>([]);

  useEffect(() => {
    const sub = pipeline431(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 431</p>
      {summary ? (
