...
export interface Sample219 {
  tick: number;
  value: number;
}

export function pipeline219(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 219 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample219, Sample219[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample219[])),
  );
}

export function windows219() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream219({ seed = 219 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample219[]>([]);

  useEffect(() => {
    const sub = pipeline219(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 219</p>
      {summary ? (
