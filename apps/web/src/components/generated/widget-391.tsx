...
export interface Sample391 {
  tick: number;
  value: number;
}

export function pipeline391(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 391 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample391, Sample391[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample391[])),
  );
}

export function windows391() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream391({ seed = 391 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample391[]>([]);

  useEffect(() => {
    const sub = pipeline391(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 391</p>
      {summary ? (
