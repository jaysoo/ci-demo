...
export interface Sample407 {
  tick: number;
  value: number;
}

export function pipeline407(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 407 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample407, Sample407[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample407[])),
  );
}

export function windows407() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream407({ seed = 407 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample407[]>([]);

  useEffect(() => {
    const sub = pipeline407(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 407</p>
      {summary ? (
