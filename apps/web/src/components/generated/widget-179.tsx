...
export interface Sample179 {
  tick: number;
  value: number;
}

export function pipeline179(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 179 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample179, Sample179[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample179[])),
  );
}

export function windows179() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream179({ seed = 179 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample179[]>([]);

  useEffect(() => {
    const sub = pipeline179(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 179</p>
      {summary ? (
