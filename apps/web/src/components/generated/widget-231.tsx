...
export interface Sample231 {
  tick: number;
  value: number;
}

export function pipeline231(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 231 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample231, Sample231[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample231[])),
  );
}

export function windows231() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream231({ seed = 231 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample231[]>([]);

  useEffect(() => {
    const sub = pipeline231(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 231</p>
      {summary ? (
