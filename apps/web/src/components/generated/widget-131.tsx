...
export interface Sample131 {
  tick: number;
  value: number;
}

export function pipeline131(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 131 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample131, Sample131[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample131[])),
  );
}

export function windows131() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream131({ seed = 131 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample131[]>([]);

  useEffect(() => {
    const sub = pipeline131(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 131</p>
      {summary ? (
