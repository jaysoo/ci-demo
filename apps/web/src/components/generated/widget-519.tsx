...
export interface Sample519 {
  tick: number;
  value: number;
}

export function pipeline519(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 519 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample519, Sample519[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample519[])),
  );
}

export function windows519() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream519({ seed = 519 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample519[]>([]);

  useEffect(() => {
    const sub = pipeline519(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 519</p>
      {summary ? (
