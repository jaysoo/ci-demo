...
export interface Sample591 {
  tick: number;
  value: number;
}

export function pipeline591(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 591 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample591, Sample591[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample591[])),
  );
}

export function windows591() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream591({ seed = 591 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample591[]>([]);

  useEffect(() => {
    const sub = pipeline591(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 591</p>
      {summary ? (
