...
export interface Panel584Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel584Data(size = 48): Panel584Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (584 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel584Datum) => d.value);

export function Panel584({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel584Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel584Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 584</h3>
