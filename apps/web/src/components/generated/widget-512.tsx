...
export interface Panel512Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel512Data(size = 48): Panel512Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (512 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel512Datum) => d.value);

export function Panel512({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel512Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel512Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 512</h3>
