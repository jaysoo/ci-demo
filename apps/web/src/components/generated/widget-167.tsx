...
export interface Sample167 {
  tick: number;
  value: number;
}

export function pipeline167(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 167 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample167, Sample167[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample167[])),
  );
}

export function windows167() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream167({ seed = 167 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample167[]>([]);

  useEffect(() => {
    const sub = pipeline167(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 167</p>
      {summary ? (
