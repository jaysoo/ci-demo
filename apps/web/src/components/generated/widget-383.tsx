...
export interface Sample383 {
  tick: number;
  value: number;
}

export function pipeline383(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 383 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample383, Sample383[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample383[])),
  );
}

export function windows383() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream383({ seed = 383 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample383[]>([]);

  useEffect(() => {
    const sub = pipeline383(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 383</p>
      {summary ? (
