...
export interface Panel244Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel244Data(size = 48): Panel244Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (244 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel244Datum) => d.value);

export function Panel244({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel244Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel244Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 244</h3>
