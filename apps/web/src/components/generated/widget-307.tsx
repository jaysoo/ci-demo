...
export interface Sample307 {
  tick: number;
  value: number;
}

export function pipeline307(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 307 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample307, Sample307[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample307[])),
  );
}

export function windows307() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream307({ seed = 307 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample307[]>([]);

  useEffect(() => {
    const sub = pipeline307(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 307</p>
      {summary ? (
