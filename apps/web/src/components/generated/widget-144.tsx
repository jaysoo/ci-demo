...
export interface Panel144Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel144Data(size = 48): Panel144Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (144 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel144Datum) => d.value);

export function Panel144({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel144Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel144Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 144</h3>
