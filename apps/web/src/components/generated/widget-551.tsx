...
export interface Sample551 {
  tick: number;
  value: number;
}

export function pipeline551(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 551 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample551, Sample551[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample551[])),
  );
}

export function windows551() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream551({ seed = 551 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample551[]>([]);

  useEffect(() => {
    const sub = pipeline551(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 551</p>
      {summary ? (
