...
export interface Sample239 {
  tick: number;
  value: number;
}

export function pipeline239(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 239 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample239, Sample239[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample239[])),
  );
}

export function windows239() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream239({ seed = 239 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample239[]>([]);

  useEffect(() => {
    const sub = pipeline239(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 239</p>
      {summary ? (
