...
export interface Sample207 {
  tick: number;
  value: number;
}

export function pipeline207(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 207 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample207, Sample207[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample207[])),
  );
}

export function windows207() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream207({ seed = 207 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample207[]>([]);

  useEffect(() => {
    const sub = pipeline207(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 207</p>
      {summary ? (
