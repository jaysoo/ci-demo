...
export interface Panel164Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel164Data(size = 48): Panel164Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (164 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel164Datum) => d.value);

export function Panel164({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel164Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel164Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 164</h3>
