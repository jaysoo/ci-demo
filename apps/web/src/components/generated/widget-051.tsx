...
export interface Sample051 {
  tick: number;
  value: number;
}

export function pipeline051(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 51 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample051, Sample051[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample051[])),
  );
}

export function windows051() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream051({ seed = 51 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample051[]>([]);

  useEffect(() => {
    const sub = pipeline051(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 051</p>
      {summary ? (
