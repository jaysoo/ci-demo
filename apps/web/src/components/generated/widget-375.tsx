...
export interface Sample375 {
  tick: number;
  value: number;
}

export function pipeline375(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 375 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample375, Sample375[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample375[])),
  );
}

export function windows375() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream375({ seed = 375 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample375[]>([]);

  useEffect(() => {
    const sub = pipeline375(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 375</p>
      {summary ? (
