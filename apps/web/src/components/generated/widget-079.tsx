...
export interface Sample079 {
  tick: number;
  value: number;
}

export function pipeline079(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 79 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample079, Sample079[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample079[])),
  );
}

export function windows079() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream079({ seed = 79 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample079[]>([]);

  useEffect(() => {
    const sub = pipeline079(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 079</p>
      {summary ? (
