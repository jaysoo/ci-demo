...
export interface Sample115 {
  tick: number;
  value: number;
}

export function pipeline115(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 115 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample115, Sample115[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample115[])),
  );
}

export function windows115() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream115({ seed = 115 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample115[]>([]);

  useEffect(() => {
    const sub = pipeline115(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 115</p>
      {summary ? (
