...
export interface Panel012Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel012Data(size = 48): Panel012Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (12 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel012Datum) => d.value);

export function Panel012({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel012Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel012Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 012</h3>
