...
export interface Sample567 {
  tick: number;
  value: number;
}

export function pipeline567(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 567 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample567, Sample567[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample567[])),
  );
}

export function windows567() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream567({ seed = 567 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample567[]>([]);

  useEffect(() => {
    const sub = pipeline567(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 567</p>
      {summary ? (
