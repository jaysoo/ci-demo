...
export interface Panel052Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel052Data(size = 48): Panel052Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (52 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel052Datum) => d.value);

export function Panel052({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel052Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel052Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 052</h3>
