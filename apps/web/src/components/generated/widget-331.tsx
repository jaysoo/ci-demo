...
export interface Sample331 {
  tick: number;
  value: number;
}

export function pipeline331(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 331 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample331, Sample331[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample331[])),
  );
}

export function windows331() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream331({ seed = 331 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample331[]>([]);

  useEffect(() => {
    const sub = pipeline331(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 331</p>
      {summary ? (
