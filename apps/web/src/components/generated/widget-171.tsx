...
export interface Sample171 {
  tick: number;
  value: number;
}

export function pipeline171(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 171 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample171, Sample171[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample171[])),
  );
}

export function windows171() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream171({ seed = 171 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample171[]>([]);

  useEffect(() => {
    const sub = pipeline171(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 171</p>
      {summary ? (
