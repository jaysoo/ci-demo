...
export interface Sample539 {
  tick: number;
  value: number;
}

export function pipeline539(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 539 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample539, Sample539[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample539[])),
  );
}

export function windows539() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream539({ seed = 539 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample539[]>([]);

  useEffect(() => {
    const sub = pipeline539(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 539</p>
      {summary ? (
