...
export interface Sample475 {
  tick: number;
  value: number;
}

export function pipeline475(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 475 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample475, Sample475[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample475[])),
  );
}

export function windows475() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream475({ seed = 475 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample475[]>([]);

  useEffect(() => {
    const sub = pipeline475(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 475</p>
      {summary ? (
