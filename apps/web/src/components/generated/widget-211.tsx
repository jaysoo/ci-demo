...
export interface Sample211 {
  tick: number;
  value: number;
}

export function pipeline211(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 211 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample211, Sample211[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample211[])),
  );
}

export function windows211() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream211({ seed = 211 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample211[]>([]);

  useEffect(() => {
    const sub = pipeline211(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 211</p>
      {summary ? (
