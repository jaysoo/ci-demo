...
export interface Panel248Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel248Data(size = 48): Panel248Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (248 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel248Datum) => d.value);

export function Panel248({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel248Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel248Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 248</h3>
