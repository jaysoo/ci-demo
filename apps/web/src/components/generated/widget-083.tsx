...
export interface Sample083 {
  tick: number;
  value: number;
}

export function pipeline083(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 83 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample083, Sample083[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample083[])),
  );
}

export function windows083() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream083({ seed = 83 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample083[]>([]);

  useEffect(() => {
    const sub = pipeline083(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 083</p>
      {summary ? (
