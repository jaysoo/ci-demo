...
export interface Sample151 {
  tick: number;
  value: number;
}

export function pipeline151(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 151 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample151, Sample151[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample151[])),
  );
}

export function windows151() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream151({ seed = 151 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample151[]>([]);

  useEffect(() => {
    const sub = pipeline151(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 151</p>
      {summary ? (
