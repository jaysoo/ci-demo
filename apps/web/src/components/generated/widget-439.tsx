...
export interface Sample439 {
  tick: number;
  value: number;
}

export function pipeline439(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 439 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample439, Sample439[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample439[])),
  );
}

export function windows439() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream439({ seed = 439 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample439[]>([]);

  useEffect(() => {
    const sub = pipeline439(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 439</p>
      {summary ? (
