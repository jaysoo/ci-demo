...
export interface Sample119 {
  tick: number;
  value: number;
}

export function pipeline119(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 119 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample119, Sample119[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample119[])),
  );
}

export function windows119() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream119({ seed = 119 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample119[]>([]);

  useEffect(() => {
    const sub = pipeline119(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 119</p>
      {summary ? (
