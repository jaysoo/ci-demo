...
export interface Sample347 {
  tick: number;
  value: number;
}

export function pipeline347(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 347 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample347, Sample347[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample347[])),
  );
}

export function windows347() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream347({ seed = 347 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample347[]>([]);

  useEffect(() => {
    const sub = pipeline347(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 347</p>
      {summary ? (
