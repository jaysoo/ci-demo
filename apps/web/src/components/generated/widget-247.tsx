...
export interface Sample247 {
  tick: number;
  value: number;
}

export function pipeline247(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 247 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample247, Sample247[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample247[])),
  );
}

export function windows247() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream247({ seed = 247 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample247[]>([]);

  useEffect(() => {
    const sub = pipeline247(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 247</p>
      {summary ? (
