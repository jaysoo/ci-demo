...
export interface Sample311 {
  tick: number;
  value: number;
}

export function pipeline311(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 311 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample311, Sample311[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample311[])),
  );
}

export function windows311() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream311({ seed = 311 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample311[]>([]);

  useEffect(() => {
    const sub = pipeline311(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 311</p>
      {summary ? (
