...
export interface Sample339 {
  tick: number;
  value: number;
}

export function pipeline339(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 339 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample339, Sample339[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample339[])),
  );
}

export function windows339() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream339({ seed = 339 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample339[]>([]);

  useEffect(() => {
    const sub = pipeline339(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 339</p>
      {summary ? (
