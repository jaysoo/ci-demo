...
export interface Sample279 {
  tick: number;
  value: number;
}

export function pipeline279(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 279 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample279, Sample279[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample279[])),
  );
}

export function windows279() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream279({ seed = 279 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample279[]>([]);

  useEffect(() => {
    const sub = pipeline279(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 279</p>
      {summary ? (
