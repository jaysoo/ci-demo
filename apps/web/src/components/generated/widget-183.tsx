...
export interface Sample183 {
  tick: number;
  value: number;
}

export function pipeline183(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 183 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample183, Sample183[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample183[])),
  );
}

export function windows183() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream183({ seed = 183 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample183[]>([]);

  useEffect(() => {
    const sub = pipeline183(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 183</p>
      {summary ? (
