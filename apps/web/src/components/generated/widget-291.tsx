...
export interface Sample291 {
  tick: number;
  value: number;
}

export function pipeline291(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 291 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample291, Sample291[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample291[])),
  );
}

export function windows291() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream291({ seed = 291 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample291[]>([]);

  useEffect(() => {
    const sub = pipeline291(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 291</p>
      {summary ? (
