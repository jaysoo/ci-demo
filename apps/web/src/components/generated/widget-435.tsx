...
export interface Sample435 {
  tick: number;
  value: number;
}

export function pipeline435(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 435 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample435, Sample435[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample435[])),
  );
}

export function windows435() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream435({ seed = 435 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample435[]>([]);

  useEffect(() => {
    const sub = pipeline435(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 435</p>
      {summary ? (
