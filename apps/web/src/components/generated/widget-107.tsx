...
export interface Sample107 {
  tick: number;
  value: number;
}

export function pipeline107(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 107 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample107, Sample107[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample107[])),
  );
}

export function windows107() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream107({ seed = 107 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample107[]>([]);

  useEffect(() => {
    const sub = pipeline107(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 107</p>
      {summary ? (
