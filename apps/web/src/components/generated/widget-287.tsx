...
export interface Sample287 {
  tick: number;
  value: number;
}

export function pipeline287(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 287 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample287, Sample287[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample287[])),
  );
}

export function windows287() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream287({ seed = 287 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample287[]>([]);

  useEffect(() => {
    const sub = pipeline287(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 287</p>
      {summary ? (
