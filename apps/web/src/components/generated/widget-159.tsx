...
export interface Sample159 {
  tick: number;
  value: number;
}

export function pipeline159(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 159 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample159, Sample159[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample159[])),
  );
}

export function windows159() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream159({ seed = 159 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample159[]>([]);

  useEffect(() => {
    const sub = pipeline159(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 159</p>
      {summary ? (
