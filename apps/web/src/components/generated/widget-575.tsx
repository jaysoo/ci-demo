...
export interface Sample575 {
  tick: number;
  value: number;
}

export function pipeline575(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 575 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample575, Sample575[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample575[])),
  );
}

export function windows575() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream575({ seed = 575 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample575[]>([]);

  useEffect(() => {
    const sub = pipeline575(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 575</p>
      {summary ? (
