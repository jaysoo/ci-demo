...
export interface Sample071 {
  tick: number;
  value: number;
}

export function pipeline071(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 71 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample071, Sample071[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample071[])),
  );
}

export function windows071() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream071({ seed = 71 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample071[]>([]);

  useEffect(() => {
    const sub = pipeline071(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 071</p>
      {summary ? (
