...
export interface Sample155 {
  tick: number;
  value: number;
}

export function pipeline155(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 155 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample155, Sample155[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample155[])),
  );
}

export function windows155() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream155({ seed = 155 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample155[]>([]);

  useEffect(() => {
    const sub = pipeline155(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 155</p>
      {summary ? (
