...
export interface Sample267 {
  tick: number;
  value: number;
}

export function pipeline267(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 267 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample267, Sample267[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample267[])),
  );
}

export function windows267() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream267({ seed = 267 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample267[]>([]);

  useEffect(() => {
    const sub = pipeline267(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 267</p>
      {summary ? (
