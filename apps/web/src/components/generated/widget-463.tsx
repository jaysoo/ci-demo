...
export interface Sample463 {
  tick: number;
  value: number;
}

export function pipeline463(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 463 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample463, Sample463[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample463[])),
  );
}

export function windows463() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream463({ seed = 463 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample463[]>([]);

  useEffect(() => {
    const sub = pipeline463(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 463</p>
      {summary ? (
