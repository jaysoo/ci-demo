...
export interface Panel452Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel452Data(size = 48): Panel452Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (452 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel452Datum) => d.value);

export function Panel452({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel452Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel452Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 452</h3>
