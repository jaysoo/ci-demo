...
export interface Sample459 {
  tick: number;
  value: number;
}

export function pipeline459(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 459 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample459, Sample459[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample459[])),
  );
}

export function windows459() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream459({ seed = 459 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample459[]>([]);

  useEffect(() => {
    const sub = pipeline459(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 459</p>
      {summary ? (
