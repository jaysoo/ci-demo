...
export interface Sample423 {
  tick: number;
  value: number;
}

export function pipeline423(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 423 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample423, Sample423[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample423[])),
  );
}

export function windows423() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream423({ seed = 423 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample423[]>([]);

  useEffect(() => {
    const sub = pipeline423(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 423</p>
      {summary ? (
