...
export interface Sample035 {
  tick: number;
  value: number;
}

export function pipeline035(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 35 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample035, Sample035[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample035[])),
  );
}

export function windows035() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream035({ seed = 35 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample035[]>([]);

  useEffect(() => {
    const sub = pipeline035(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 035</p>
      {summary ? (
