...
export interface Sample011 {
  tick: number;
  value: number;
}

export function pipeline011(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 11 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample011, Sample011[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample011[])),
  );
}

export function windows011() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream011({ seed = 11 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample011[]>([]);

  useEffect(() => {
    const sub = pipeline011(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 011</p>
      {summary ? (
