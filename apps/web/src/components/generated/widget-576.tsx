...
export interface Panel576Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel576Data(size = 48): Panel576Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (576 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel576Datum) => d.value);

export function Panel576({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel576Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel576Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 576</h3>
