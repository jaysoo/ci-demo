...
export interface Sample215 {
  tick: number;
  value: number;
}

export function pipeline215(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 215 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample215, Sample215[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample215[])),
  );
}

export function windows215() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream215({ seed = 215 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample215[]>([]);

  useEffect(() => {
    const sub = pipeline215(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 215</p>
      {summary ? (
