...
export interface Sample371 {
  tick: number;
  value: number;
}

export function pipeline371(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 371 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample371, Sample371[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample371[])),
  );
}

export function windows371() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream371({ seed = 371 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample371[]>([]);

  useEffect(() => {
    const sub = pipeline371(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 371</p>
      {summary ? (
