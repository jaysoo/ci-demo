...
export interface Sample275 {
  tick: number;
  value: number;
}

export function pipeline275(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 275 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample275, Sample275[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample275[])),
  );
}

export function windows275() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream275({ seed = 275 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample275[]>([]);

  useEffect(() => {
    const sub = pipeline275(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 275</p>
      {summary ? (
