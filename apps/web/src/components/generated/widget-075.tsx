...
export interface Sample075 {
  tick: number;
  value: number;
}

export function pipeline075(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 75 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample075, Sample075[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample075[])),
  );
}

export function windows075() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream075({ seed = 75 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample075[]>([]);

  useEffect(() => {
    const sub = pipeline075(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 075</p>
      {summary ? (
