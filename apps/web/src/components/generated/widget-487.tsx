...
export interface Sample487 {
  tick: number;
  value: number;
}

export function pipeline487(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 487 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample487, Sample487[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample487[])),
  );
}

export function windows487() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream487({ seed = 487 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample487[]>([]);

  useEffect(() => {
    const sub = pipeline487(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 487</p>
      {summary ? (
