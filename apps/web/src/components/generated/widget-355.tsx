...
export interface Sample355 {
  tick: number;
  value: number;
}

export function pipeline355(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 355 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample355, Sample355[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample355[])),
  );
}

export function windows355() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream355({ seed = 355 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample355[]>([]);

  useEffect(() => {
    const sub = pipeline355(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 355</p>
      {summary ? (
