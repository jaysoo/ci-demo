...
export interface Sample559 {
  tick: number;
  value: number;
}

export function pipeline559(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 559 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample559, Sample559[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample559[])),
  );
}

export function windows559() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream559({ seed = 559 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample559[]>([]);

  useEffect(() => {
    const sub = pipeline559(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 559</p>
      {summary ? (
