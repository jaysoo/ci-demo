...
export interface Sample227 {
  tick: number;
  value: number;
}

export function pipeline227(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 227 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample227, Sample227[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample227[])),
  );
}

export function windows227() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream227({ seed = 227 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample227[]>([]);

  useEffect(() => {
    const sub = pipeline227(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 227</p>
      {summary ? (
