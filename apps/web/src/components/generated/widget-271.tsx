...
export interface Sample271 {
  tick: number;
  value: number;
}

export function pipeline271(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 271 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample271, Sample271[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample271[])),
  );
}

export function windows271() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream271({ seed = 271 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample271[]>([]);

  useEffect(() => {
    const sub = pipeline271(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 271</p>
      {summary ? (
