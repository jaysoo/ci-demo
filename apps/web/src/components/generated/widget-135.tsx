...
export interface Sample135 {
  tick: number;
  value: number;
}

export function pipeline135(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 135 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample135, Sample135[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample135[])),
  );
}

export function windows135() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream135({ seed = 135 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample135[]>([]);

  useEffect(() => {
    const sub = pipeline135(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 135</p>
      {summary ? (
