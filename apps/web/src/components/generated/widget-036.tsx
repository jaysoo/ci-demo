...
export interface Panel036Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel036Data(size = 48): Panel036Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (36 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel036Datum) => d.value);

export function Panel036({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel036Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel036Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 036</h3>
