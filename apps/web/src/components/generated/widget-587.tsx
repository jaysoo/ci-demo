...
export interface Sample587 {
  tick: number;
  value: number;
}

export function pipeline587(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 587 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample587, Sample587[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample587[])),
  );
}

export function windows587() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream587({ seed = 587 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample587[]>([]);

  useEffect(() => {
    const sub = pipeline587(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 587</p>
      {summary ? (
