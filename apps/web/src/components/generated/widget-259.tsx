...
export interface Sample259 {
  tick: number;
  value: number;
}

export function pipeline259(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 259 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample259, Sample259[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample259[])),
  );
}

export function windows259() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream259({ seed = 259 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample259[]>([]);

  useEffect(() => {
    const sub = pipeline259(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 259</p>
      {summary ? (
