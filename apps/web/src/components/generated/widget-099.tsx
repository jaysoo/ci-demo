...
export interface Sample099 {
  tick: number;
  value: number;
}

export function pipeline099(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 99 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample099, Sample099[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample099[])),
  );
}

export function windows099() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream099({ seed = 99 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample099[]>([]);

  useEffect(() => {
    const sub = pipeline099(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 099</p>
      {summary ? (
