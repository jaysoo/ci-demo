...
export interface Sample055 {
  tick: number;
  value: number;
}

export function pipeline055(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 55 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample055, Sample055[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample055[])),
  );
}

export function windows055() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream055({ seed = 55 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample055[]>([]);

  useEffect(() => {
    const sub = pipeline055(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 055</p>
      {summary ? (
