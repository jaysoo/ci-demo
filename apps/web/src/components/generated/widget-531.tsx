...
export interface Sample531 {
  tick: number;
  value: number;
}

export function pipeline531(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 531 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample531, Sample531[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample531[])),
  );
}

export function windows531() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream531({ seed = 531 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample531[]>([]);

  useEffect(() => {
    const sub = pipeline531(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 531</p>
      {summary ? (
