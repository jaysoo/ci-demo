...
export interface Sample535 {
  tick: number;
  value: number;
}

export function pipeline535(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 535 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample535, Sample535[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample535[])),
  );
}

export function windows535() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream535({ seed = 535 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample535[]>([]);

  useEffect(() => {
    const sub = pipeline535(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 535</p>
      {summary ? (
