...
export interface Sample323 {
  tick: number;
  value: number;
}

export function pipeline323(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 323 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample323, Sample323[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample323[])),
  );
}

export function windows323() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream323({ seed = 323 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample323[]>([]);

  useEffect(() => {
    const sub = pipeline323(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 323</p>
      {summary ? (
