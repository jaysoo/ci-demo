...
export interface Sample263 {
  tick: number;
  value: number;
}

export function pipeline263(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 263 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample263, Sample263[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample263[])),
  );
}

export function windows263() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream263({ seed = 263 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample263[]>([]);

  useEffect(() => {
    const sub = pipeline263(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 263</p>
      {summary ? (
