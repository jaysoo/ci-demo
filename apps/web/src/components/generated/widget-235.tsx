...
export interface Sample235 {
  tick: number;
  value: number;
}

export function pipeline235(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 235 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample235, Sample235[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample235[])),
  );
}

export function windows235() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream235({ seed = 235 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample235[]>([]);

  useEffect(() => {
    const sub = pipeline235(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 235</p>
      {summary ? (
