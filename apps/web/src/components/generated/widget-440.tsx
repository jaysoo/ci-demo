...
export interface Panel440Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel440Data(size = 48): Panel440Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (440 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel440Datum) => d.value);

export function Panel440({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel440Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel440Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 440</h3>
