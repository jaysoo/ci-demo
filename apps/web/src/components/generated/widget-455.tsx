...
export interface Sample455 {
  tick: number;
  value: number;
}

export function pipeline455(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 455 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample455, Sample455[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample455[])),
  );
}

export function windows455() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream455({ seed = 455 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample455[]>([]);

  useEffect(() => {
    const sub = pipeline455(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 455</p>
      {summary ? (
