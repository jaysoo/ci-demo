...
export interface Sample295 {
  tick: number;
  value: number;
}

export function pipeline295(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 295 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample295, Sample295[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample295[])),
  );
}

export function windows295() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream295({ seed = 295 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample295[]>([]);

  useEffect(() => {
    const sub = pipeline295(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 295</p>
      {summary ? (
