...
export interface Sample175 {
  tick: number;
  value: number;
}

export function pipeline175(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 175 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample175, Sample175[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample175[])),
  );
}

export function windows175() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream175({ seed = 175 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample175[]>([]);

  useEffect(() => {
    const sub = pipeline175(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 175</p>
      {summary ? (
