...
export interface Panel104Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel104Data(size = 48): Panel104Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (104 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel104Datum) => d.value);

export function Panel104({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel104Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel104Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 104</h3>
