...
export interface Panel152Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel152Data(size = 48): Panel152Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (152 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel152Datum) => d.value);

export function Panel152({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel152Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel152Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 152</h3>
