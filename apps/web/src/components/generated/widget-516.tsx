...
export interface Panel516Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel516Data(size = 48): Panel516Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (516 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel516Datum) => d.value);

export function Panel516({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel516Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel516Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 516</h3>
