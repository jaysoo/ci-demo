...
export interface Panel328Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel328Data(size = 48): Panel328Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (328 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel328Datum) => d.value);

export function Panel328({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel328Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel328Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 328</h3>
