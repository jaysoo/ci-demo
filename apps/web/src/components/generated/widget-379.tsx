...
export interface Sample379 {
  tick: number;
  value: number;
}

export function pipeline379(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 379 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample379, Sample379[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample379[])),
  );
}

export function windows379() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream379({ seed = 379 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample379[]>([]);

  useEffect(() => {
    const sub = pipeline379(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 379</p>
      {summary ? (
