...
export interface Panel540Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel540Data(size = 48): Panel540Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (540 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel540Datum) => d.value);

export function Panel540({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel540Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel540Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 540</h3>
