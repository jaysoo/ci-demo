...
export interface Sample059 {
  tick: number;
  value: number;
}

export function pipeline059(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 59 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample059, Sample059[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample059[])),
  );
}

export function windows059() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream059({ seed = 59 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample059[]>([]);

  useEffect(() => {
    const sub = pipeline059(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 059</p>
      {summary ? (
