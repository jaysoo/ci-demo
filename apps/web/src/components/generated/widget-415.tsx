...
export interface Sample415 {
  tick: number;
  value: number;
}

export function pipeline415(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 415 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample415, Sample415[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample415[])),
  );
}

export function windows415() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream415({ seed = 415 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample415[]>([]);

  useEffect(() => {
    const sub = pipeline415(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 415</p>
      {summary ? (
