...
export interface Sample063 {
  tick: number;
  value: number;
}

export function pipeline063(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 63 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample063, Sample063[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample063[])),
  );
}

export function windows063() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream063({ seed = 63 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample063[]>([]);

  useEffect(() => {
    const sub = pipeline063(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 063</p>
      {summary ? (
