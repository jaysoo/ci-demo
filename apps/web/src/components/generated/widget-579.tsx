...
export interface Sample579 {
  tick: number;
  value: number;
}

export function pipeline579(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 579 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample579, Sample579[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample579[])),
  );
}

export function windows579() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream579({ seed = 579 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample579[]>([]);

  useEffect(() => {
    const sub = pipeline579(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 579</p>
      {summary ? (
