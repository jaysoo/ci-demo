...
export interface Sample103 {
  tick: number;
  value: number;
}

export function pipeline103(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 103 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample103, Sample103[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample103[])),
  );
}

export function windows103() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream103({ seed = 103 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample103[]>([]);

  useEffect(() => {
    const sub = pipeline103(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 103</p>
      {summary ? (
