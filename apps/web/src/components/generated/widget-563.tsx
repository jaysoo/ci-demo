...
export interface Sample563 {
  tick: number;
  value: number;
}

export function pipeline563(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 563 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample563, Sample563[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample563[])),
  );
}

export function windows563() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream563({ seed = 563 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample563[]>([]);

  useEffect(() => {
    const sub = pipeline563(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 563</p>
      {summary ? (
