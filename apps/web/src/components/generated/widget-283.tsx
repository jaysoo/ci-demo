...
export interface Sample283 {
  tick: number;
  value: number;
}

export function pipeline283(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 283 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample283, Sample283[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample283[])),
  );
}

export function windows283() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream283({ seed = 283 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample283[]>([]);

  useEffect(() => {
    const sub = pipeline283(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 283</p>
      {summary ? (
