...
export interface Sample015 {
  tick: number;
  value: number;
}

export function pipeline015(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 15 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample015, Sample015[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample015[])),
  );
}

export function windows015() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream015({ seed = 15 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample015[]>([]);

  useEffect(() => {
    const sub = pipeline015(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 015</p>
      {summary ? (
