...
export interface Panel240Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel240Data(size = 48): Panel240Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (240 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel240Datum) => d.value);

export function Panel240({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel240Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel240Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 240</h3>
