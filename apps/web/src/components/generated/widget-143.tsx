...
export interface Sample143 {
  tick: number;
  value: number;
}

export function pipeline143(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 143 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample143, Sample143[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample143[])),
  );
}

export function windows143() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream143({ seed = 143 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample143[]>([]);

  useEffect(() => {
    const sub = pipeline143(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 143</p>
      {summary ? (
