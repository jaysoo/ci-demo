...
export interface Sample495 {
  tick: number;
  value: number;
}

export function pipeline495(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 495 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample495, Sample495[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample495[])),
  );
}

export function windows495() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream495({ seed = 495 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample495[]>([]);

  useEffect(() => {
    const sub = pipeline495(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 495</p>
      {summary ? (
