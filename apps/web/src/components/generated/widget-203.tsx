...
export interface Sample203 {
  tick: number;
  value: number;
}

export function pipeline203(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 203 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample203, Sample203[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample203[])),
  );
}

export function windows203() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream203({ seed = 203 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample203[]>([]);

  useEffect(() => {
    const sub = pipeline203(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 203</p>
      {summary ? (
