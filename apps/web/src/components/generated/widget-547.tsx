...
export interface Sample547 {
  tick: number;
  value: number;
}

export function pipeline547(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 547 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample547, Sample547[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample547[])),
  );
}

export function windows547() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream547({ seed = 547 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample547[]>([]);

  useEffect(() => {
    const sub = pipeline547(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 547</p>
      {summary ? (
