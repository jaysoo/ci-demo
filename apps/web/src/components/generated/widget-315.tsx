...
export interface Sample315 {
  tick: number;
  value: number;
}

export function pipeline315(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 315 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample315, Sample315[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample315[])),
  );
}

export function windows315() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream315({ seed = 315 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample315[]>([]);

  useEffect(() => {
    const sub = pipeline315(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 315</p>
      {summary ? (
