...
export interface Sample511 {
  tick: number;
  value: number;
}

export function pipeline511(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 511 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample511, Sample511[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample511[])),
  );
}

export function windows511() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream511({ seed = 511 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample511[]>([]);

  useEffect(() => {
    const sub = pipeline511(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 511</p>
      {summary ? (
