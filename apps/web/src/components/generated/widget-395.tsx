...
export interface Sample395 {
  tick: number;
  value: number;
}

export function pipeline395(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 395 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample395, Sample395[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample395[])),
  );
}

export function windows395() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream395({ seed = 395 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample395[]>([]);

  useEffect(() => {
    const sub = pipeline395(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 395</p>
      {summary ? (
