...
export interface Sample447 {
  tick: number;
  value: number;
}

export function pipeline447(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 447 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample447, Sample447[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample447[])),
  );
}

export function windows447() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream447({ seed = 447 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample447[]>([]);

  useEffect(() => {
    const sub = pipeline447(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 447</p>
      {summary ? (
