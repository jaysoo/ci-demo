...
export interface Sample299 {
  tick: number;
  value: number;
}

export function pipeline299(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 299 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample299, Sample299[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample299[])),
  );
}

export function windows299() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream299({ seed = 299 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample299[]>([]);

  useEffect(() => {
    const sub = pipeline299(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 299</p>
      {summary ? (
