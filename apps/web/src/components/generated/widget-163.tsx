...
export interface Sample163 {
  tick: number;
  value: number;
}

export function pipeline163(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 163 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample163, Sample163[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample163[])),
  );
}

export function windows163() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream163({ seed = 163 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample163[]>([]);

  useEffect(() => {
    const sub = pipeline163(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 163</p>
      {summary ? (
