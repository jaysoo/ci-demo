...
export interface Sample387 {
  tick: number;
  value: number;
}

export function pipeline387(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 387 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample387, Sample387[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample387[])),
  );
}

export function windows387() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream387({ seed = 387 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample387[]>([]);

  useEffect(() => {
    const sub = pipeline387(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 387</p>
      {summary ? (
