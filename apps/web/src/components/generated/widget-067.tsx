...
export interface Sample067 {
  tick: number;
  value: number;
}

export function pipeline067(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 67 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample067, Sample067[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample067[])),
  );
}

export function windows067() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream067({ seed = 67 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample067[]>([]);

  useEffect(() => {
    const sub = pipeline067(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 067</p>
      {summary ? (
