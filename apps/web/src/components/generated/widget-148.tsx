...
export interface Panel148Datum {
  label: string;
  value: number;
  secondary: number;
  bucket: string;
...
export function makePanel148Data(size = 48): Panel148Datum[] {
  return Array.from({ length: size }, (_, i) => ({
    label: `p${i}`,
    value: Math.sin(i / (148 % 7 || 3)) * 100 + 120,
...
const byValue = bisector((d: Panel148Datum) => d.value);

export function Panel148({ size = 48 }: { size?: number }) {
  const data = useMemo(() => makePanel148Data(size), [size]);
  ...
  const stats = useMemo(() => {
    ...
    const bins = histogram<Panel148Datum, number>()
      .value((d) => d.value)
      .domain(domain)(data);
    return {
      ranked,
  ...
  return (
    <section className="rounded-2xl border border-black/5 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-sm font-semibold">Panel 148</h3>
