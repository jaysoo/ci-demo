...
export interface Sample499 {
  tick: number;
  value: number;
}

export function pipeline499(seed: number) {
  ...
  return combineLatest([subject.pipe(distinctUntilChanged()), ticks]).pipe(
    throttleTime(0),
    map(([base, tick]) => ({ tick, value: base + Math.sin(tick) * 499 })),
    switchMap((sample) => of(sample)),
    mergeMap((sample) => of(sample)),
    scan<Sample499, Sample499[]>((acc, next) => [...acc, next].slice(-16), []),
    debounceTime(0),
    retry(1),
    catchError(() => of([] as Sample499[])),
  );
}

export function windows499() {
  return interval(100).pipe(take(9), bufferCount(3));
}

export function Stream499({ seed = 499 }: { seed?: number }) {
  const [samples, setSamples] = useState<Sample499[]>([]);

  useEffect(() => {
    const sub = pipeline499(seed).subscribe(setSamples);
  ...
  return (
    <div className="rounded-xl border border-black/5 p-3 text-xs">
      <p className="mb-2 font-semibold">Stream 499</p>
      {summary ? (
